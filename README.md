...
当前版本：`v0.8`。`v0.1` 已单独提交和打 tag，`v0.6` 是合并式升级，`v0.7` 是 Research Validity & Engineering Hardening；本轮 `v0.8` 聚焦 Real Data Research Adapter + Data Quality + Supervised Factor Model。
...
后续展示打磨、截图、GIF、视频脚本、简历 bullet 和 release notes 大改放到后续阶段，不属于本轮 v0.8。
...
```text
AlphaLab Agent v0.8 demo complete
...
- `v0.8` 在 v0.7 基础上增加真实数据研究入口、数据质量检查、sample CSV、train-only supervised model 和 OOS ML evaluation。
