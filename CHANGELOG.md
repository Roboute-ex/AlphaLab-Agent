...
- Added `docs/RUNBOOK.md` for local validation, expected outputs, release routine, and release blockers.
- Added `docs/TROUBLESHOOTING.md` for common local, optional dependency, CI, version, and artifact issues.
- Added `docs/CONFIG_GUIDE.md` for deterministic configuration usage.
- Added `examples/config_synthetic_minimal.json`.
- Added `examples/config_csv_sample.json`.
- Added conservative Dependabot configuration for monthly pip dependency PRs.
- Added tests for agent documentation and README agent sections.
- Added maintenance docs, config examples, and artifact hygiene tests.
...
- Expanded README roadmap, project structure, and quickstart maintenance guidance.
- Expanded maintenance docs to cover generated artifacts, release checks, optional dependency boundaries, Dependabot, and version consistency.
...
- No changes to core research logic, synthetic data, portfolio construction, benchmark calculation, walk-forward split, factor formulas, or Reviewer severity rules.
