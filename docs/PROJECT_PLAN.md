...
- 当前 `v0.10` 是 Agent Design Transparency and Maintenance Hygiene。
- 后续再做 Public Demo Polish；不要把截图、视频脚本、简历 bullet、release notes 大改写成当前已完成。

## 2. v0.10 目标

把项目从 `v0.9` 的维护治理版本，升级为 Agent 设计更透明、README 更完整、demo 更容易复现、维护流程更清晰的成熟项目文档版本。v0.10 不是策略能力升级，而是 Agent design transparency、Runbook、Troubleshooting、Config guide、Config examples、Artifact hygiene 和 Documentation tests 的合并维护版本。
...
- `docs/RUNBOOK.md`: local validation commands, expected outputs, release routine, and release blockers。
- `docs/TROUBLESHOOTING.md`: common local, optional dependency, CI, version, and artifact issues。
- `docs/CONFIG_GUIDE.md`: deterministic config examples and safety boundaries。
...
- `examples/config_synthetic_minimal.json`: minimal deterministic synthetic config。
- `examples/config_csv_sample.json`: documented CSV sample workflow config plus CLI path metadata。
...
- runbook, troubleshooting, and config guide documentation。
- config examples are valid JSON and contain no sensitive fields。
- `.gitignore` 对 `artifacts/run_manifest.json` 的忽略规则。
- generated artifacts 不应提交的说明。
- CI 文件存在且命令合理。
