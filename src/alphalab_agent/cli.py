...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ticker", help="Backward-compatible single ticker for --data-source yfinance.")
    parser.add_argument("--tickers", help="Comma-separated tickers for --data-source yfinance.")
    parser.add_argument("--start", help="Backward-compatible start date for --data-source yfinance, YYYY-MM-DD.")
    parser.add_argument("--end", help="Backward-compatible end date for --data-source yfinance, YYYY-MM-DD.")
    parser.add_argument("--start-date", help="Start date for --data-source yfinance, YYYY-MM-DD.")
    parser.add_argument("--end-date", help="End date for --data-source yfinance, YYYY-MM-DD.")
    parser.add_argument("--interval", default="1d", help="Interval for --data-source yfinance.")
    parser.add_argument("--enable-supervised-model", action="store_true", help="Enable train-only supervised factor model.")
    parser.add_argument("--model-type", choices=["ridge", "linear"], help="Supervised factor model type.")
    parser.add_argument(
        "--backtest-mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.enable_supervised_model:
        overrides["enable_supervised_model"] = True
    if args.model_type:
    ...
    raw_data = load_market_data(
        ...
        start=args.start_date or args.start,
        end=args.end_date or args.end,
    ...
    if workflow_artifacts is not None:
        ...
        print(
            ...
            "optional supervised model -> sensitivity -> reviewer -> report"
        )
